...
def get_topk_features(X, y, k):
    ...
    X_u = X[y==1]
    X_l = X[y==0]
    print(X_u.shape, X_l.shape)
    X_u = X_u.mean(axis=0)
    X_l = X_l.mean(axis=0)
    abs_diff = np.abs(X_u - X_l)
    topk_features = np.argsort(abs_diff)[-k:]
...
def get_important_features(X, pred, plddt, tm_score, thresholds, directions='upper'):
    ...
    def get_important_features(X, pred, thresholds, directions):
        """
        Process the LR probe
        """
        y = get_mask(pred, thresholds, directions)


        topk_features = get_topk_features(X, y, 20)
        return topk_features


    # Prediction 1
    topk_features_pred = get_important_features(X, pred, thresholds['pred'], directions['pred'])


    # PLDDT 
    topk_features_plddt = get_important_features(X, plddt, thresholds['plddt'], directions['plddt'])

    # TM-score
    topk_features_tm_score = get_important_features(X, tm_score, thresholds['tm_score'], directions['tm_score'])





    return topk_features_pred, topk_features_plddt, topk_features_tm_score
...
def analyze_important_features(mean_features, activity, plddt, tm_score, thresholds, direction):
    ...
    topk_features_pred, topk_features_plddt, topk_features_tm_score = get_important_features(mean_features, activity, plddt, tm_score, thresholds, direction)
    unique_coefs = np.stack([topk_features_pred, topk_features_plddt, topk_features_tm_score])
...
if __name__ == "__main__":
    ...
    os.makedirs(f"{output_dir}/important_features_topk", exist_ok=True)
    with open(f"{output_dir}/important_features_topk/important_features_topk_pos_M{model_iteration}_D{data_iteration}.pkl", "wb") as f:
        pkl.dump(importance_features_pos, f)
    with open(f"{output_dir}/important_features_topk/important_features_topk_neg_M{model_iteration}_D{data_iteration}.pkl", "wb") as f:
