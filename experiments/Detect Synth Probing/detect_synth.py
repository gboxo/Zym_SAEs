...
def get_natural_and_synth_sequences():
    ...
    with open("Data/Detect_Synth_Data/natural_sequences_by_ec.json", "r") as f:
        data = json.load(f)
    ...
    files = os.listdir("Data/Detect_Synth_Data/multiple_labels")
    files = [file for file in files if file.endswith(".fasta")]
    ec_synth = [key.split("_")[0] for key in files]
    synth_sequences = []
    for file in files:
        with open(f"Data/Detect_Synth_Data/multiple_labels/{file}", "r") as f:
            seq = f.read()
            seq = seq.split("\n")[1].strip("<en")
...
if __name__ == "__main__":
    ...
    with open("Data/Detect_Synth_Data/results/testing_table.txt", "w") as f:
