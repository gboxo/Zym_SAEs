...
def ablation(activations:torch.Tensor, hook, steering_vector:torch.Tensor):
    # Check prompt processing
    activations = activations + steering_vector * 10
...
def generate_with_steering(model: HookedTransformer, prompt: str, steering_vector: torch.Tensor, max_new_tokens=256, n_samples=10):
...
def get_steering_vector(model: HookedTransformer, prompt: str, df:pd.DataFrame):
    ...
    # Get top 25 sequences

    sequences_top_25 = sequences[activity > thresholds_pos["pred"]]
    tokenized_top_25 = [model.to_tokens(prompt + s) for s in sequences_top_25]
    sequences_bottom_25 = sequences[activity < thresholds_neg["pred"]]
    tokenized_bottom_25 = [model.to_tokens(prompt + s) for s in sequences_bottom_25]
    ...
    with torch.no_grad():
        for i in range(len(tokenized_top_25)):
            logits,cache_pos = model.run_with_cache(tokenized_top_25[i], names_filter=names_filter)
            acts_pos = cache_pos["blocks.25.hook_resid_pre"][0]
            all_pos.append(acts_pos.mean(dim=0))
        for i in range(len(tokenized_bottom_25)):
            logits,cache_neg = model.run_with_cache(tokenized_bottom_25[i], names_filter=names_filter)
...
if __name__ == "__main__":
    ...
    out = generate_with_steering(model, prompt, steering_vector, max_new_tokens=1014, n_samples=20)
    with open(f"{out_dir}/steering_vector_10.txt", "w") as f:
